...
export class Launify implements INodeType {
				...
				options: [
					{
						...
						routing: {
							request: {
								...
								headers: {
									'Referer': '={{ (($credentials.referer.includes("://") ? "" : (($credentials.referer.includes("localhost") || $credentials.referer.includes("127.0.0.1")) ? "http://" : "https://")) + $credentials.referer).replace(/\/$/, "") }}',
									'Origin': '={{ (($credentials.referer.includes("://") ? "" : (($credentials.referer.includes("localhost") || $credentials.referer.includes("127.0.0.1")) ? "http://" : "https://")) + $credentials.referer).replace(/\/$/, "") }}',
								},
							},
						},
					},
